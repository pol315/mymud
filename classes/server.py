...
Contains one class, _Server, which can be instantiated to start a
...
class _Server(object):
	...
	def __init__(self):
		"""Constructs the _Server object and starts listening for
		new players.
		"""
		...
		self._listen_socket.bind(("0.0.0.0", 6969))
	...
	def send_message(self, to, message, effect = "0", colour = "37", nr = True):
		"""Sends the text in the 'message' parameter to the player with
		the id number given in the 'to' parameter. The text will be
		printed out in the player's terminal. Style the text with 
		effect and colour, and decide if we want a new line character with xnr
		"""
		
		tosend = "\033[" + colour + ";" + effect + "m" + message
		if nr:
			tosend = tosend + "\n\r"
			
		tosend = tosend + "\033[0m"
		
		self._attempt_send(to, tosend)			
		
	def obscure_input(self, to):
		"""Sends 3 bytes that signal the client should start hiding
		input. See http://www.faqs.org/rfcs/rfc854.html .
		"""
		# RFC 854
		self._attempt_send(to, "\xff\xfb\x01")
		
	def show_input(self, to):
		"""Sends 3 bytes that signal the client should start showing
		input. See http://www.faqs.org/rfcs/rfc857.html .
		"""
		# RFC 857
		self._attempt_send(to, "\xff\xfc\x01")
		
	def terminate_connection(self, clid):
		"""Closes a client socket.
		"""
		self._clients[clid].socket.shutdown(socket.SHUT_RDWR)
		self._clients[clid].socket.close()
	...
	def _check_for_new_connections(self):
		...
		self._clients[self._nextid] = _Server._Client(joined_socket, addr[0],
														"", time.time())
